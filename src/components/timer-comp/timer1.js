 ...
 */
export default class PCountdown extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return renderer({...this.state, ...formattedDelta });
  }
}
...
const renderer = ({ hours, minutes, seconds, completed }) => {
  if (completed) {
    return <Completionist />;     // Render a complete state
  } else {
    return <span>{hours}:{minutes}:{seconds}</span>;// Render a countdown
  }
};
PCountdown.propTypes = {
